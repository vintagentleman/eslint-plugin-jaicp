...
function getScriptStartBlock(line, index) {
  // Does a script block begin on the current line?
  const match = line.match(SCRIPT_BLOCK_START_REGEX);
  if (match === null) return null;

  const block = new ScriptBlock(match.groups.tag, index + 1, match.groups.indent.length); // Lines are 1-indexed
...
function getScriptBlocks(text) {
  ...
  lines.forEach((line, index) => {
    ...
    if (currentBlock === null) {
      currentBlock = getScriptStartBlock(line, index);
      return;
    }
    ...
    /**
     * The block only ends if:
     * 1) The line begins with a tag or a comment.
     * 2) Its indentation level is *lower* than that of the script tag.
     */
    if ((match.groups.tag || match.groups.comment) && match.groups.indent.length <= currentBlock.tagColumn) {
    ...
    // The block ends if the current line is the last one in the file.
