...
class ScriptBlock {
  constructor(tagName, tagLine, tagColumn) {
    ...
    this.indentedScript = undefined; // {indent: number, content: string}
  }

  toString() {
    if (this.tagLineScript === undefined && this.indentedScript === undefined) return "";
    if (this.tagLineScript === undefined) return this.indentedScript.content;
    if (this.indentedScript === undefined) return this.tagLineScript.content + "\n";
    return [this.tagLineScript.content, this.indentedScript.content].join("\n");
  }
}
...
function preprocess(text, filename) {
  ...
  return file.data.scriptBlocks.map((block) => block.toString());
}
